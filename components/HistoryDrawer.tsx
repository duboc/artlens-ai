        ...
        <div className="overflow-y-auto h-full pb-32 no-scrollbar">
          {history.length === 0 ? (
              ...
              history.map((item) => (
                  ...
                  >
                      <HistoryThumbnail imageUrl={item.imageUrl} title={item.data.title} />
...
const HistoryThumbnail = ({ imageUrl, title }: { imageUrl: string; title: string }) => {
  const [failed, setFailed] = useState(false);

  if (failed || !imageUrl) {
    return (
      <div className="w-14 h-14 rounded-lg border border-[var(--primary-dim)] bg-[var(--surface)] flex items-center justify-center flex-shrink-0">
  ...
  return (
    <img
      src={imageUrl}
      alt={title}
      className="w-14 h-14 object-cover rounded-lg border border-[var(--primary-dim)] bg-[var(--surface)] flex-shrink-0"
